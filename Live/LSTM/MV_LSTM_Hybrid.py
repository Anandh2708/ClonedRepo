...
class MV_LSTM_Hybrid:
    ...
    for i in range(len(df)-1):
        if(open_values[i+1]-open_values[i]) > 0:
            news.append(1)
        else:
            news.append(0)
    temp = News_Prediction()
    temp = temp.news_prediction()
    news.append(temp[0])
    df['news'] = news
    #------------------------------------------------
    ...
    def clt_LSTM_Model(self):
        ...
        FEATURES = ['High', 'Low', 'Open', 'Close', 'Volume', 'interest', 'Crude Oil', 'news']
        #FEATURES = ['High', 'Low', 'Open', 'Close', 'Volume', 'news']
        ...
        #model.save('MVLSTM_hybrid.h5')
        model.save(model_path+'\MVLSTM_hybrid.h5')
    ...
    def load_Test_Model(self):
        ...
        # Model Training
        #model = load_model('MVLSTM_hybrid.h5')
        model = load_model(model_path+'\MVLSTM_hybrid.h5')
        ...
        save = save[select]
        # print(save)
        filename = os.path.join(rp.dirname, 'Medium/LSTM_hybrid.csv')
